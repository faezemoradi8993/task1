    ...
    <div className="flex flex-col items-end">
      ...
      <div className="flex flex-col w-full ">
        {data?.data?.items.length === 0 ? (
          <div className="w-full h-[300px] flex items-center justify-center">هیچ کالایی در سبد شما وجود ندارد</div>
        ) : (
          <div>
            {data?.data?.items.map((p) => (
              <div key={p?.key} className="flex items-center justify-between bg-white mb-2 px-2 hover:shadow-md">
                <Button
                ...
                <div className="flex flex-1 justify-between items-center  text-[11px] py-2 pl-2 ">
                  <h2 className="mr-2 bg-gray-100 p-1 ">قیمت کل : {p?.total}</h2>
                  <h2 className="mr-2 bg-gray-100 p-1 ">تعداد : {p?.quantity}</h2>
                  <h2 className="mr-2 bg-gray-100 p-1 ">قیمت واحد : {p?.price}</h2>
            ...
            <div className="mt-3 border-t-2 border-t-blue-200 w-full">
              <div className="flex items-center justify-between bg-white mb-2 p-2 ">
                ...
                <div className="flex flex-1 justify-end items-center text-[11px] py-2  ">
                  <h2 className="mr-2 bg-gray-100 p-1 ">قیمت کل : {data?.data?.total_price}</h2>
                  <h2 className="mr-2 bg-gray-100 p-1 ">تعداد : {data?.data?.total_count}</h2>
