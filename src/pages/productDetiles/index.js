    ...
    <div className="flex flex-col items-end">
      ...
      <div className="my-5 p-5 relative w-full  bg-white shadow-lg">
        <div className="w-[120px] h-[120px] bg-white mx-auto">
          <img className="w-[120px] mx-auto " src={data?.data?.data?.thumbnail} alt={data?.data?.data?.title} />
        </div>
        ...
        <h1 className="border-blue-100 font-bold text-sm text-blue-900 text-right mt-3">{data?.data?.data?.title}</h1>
        <h2 className="text-[11px]  text-right text-gray-500 ">{data?.data?.data?.second_title}</h2>
        <h1 className="text-[11px]  text-right text-gray-400 ">شناسه محصول: {data?.data?.data?.id}</h1>
        <h1 className="text-left"> تومان {data?.data?.data?.price}</h1>
      </div>
      <div className="bg-white flex-1  p-5 w-full shadow-lg">
        <h3 className="border-2 border-transparent border-b-blue-200 text-right font-semibold pb-4 mb-4">مشخصات کلی</h3>
